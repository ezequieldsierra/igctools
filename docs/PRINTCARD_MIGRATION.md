...
Sólo cuando coinciden los seis archivos auditados, los hooks sustituyen el controlador completo y siete rutas de generación, firma
...
| Activación | Fuente desconocida conserva los hooks anteriores; metadata incompatible y hooks mezclados se rechazan |
| Migración con PowerPro distinto | Se modifican los tres archivos del incidente y se ejecuta `SiteMigration.pre_schema_updates()` real sin activar IGCTools ni cambiar adjuntos |
...
La regresión de migración usa un controlador estándar de Frappe como motor previo
del DocType de prueba; no acredita compatibilidad con una versión privada de PowerPro.
...
**La compatibilidad se decide antes de registrar los hooks de PrintCard.** Si
PowerPro falta o alguno de sus seis archivos difiere, IGCTools no registra su
controlador, sus siete rutas ni su filtro de permisos. El motor anterior sigue
atendiendo todos los PDF y las capas nuevas quedan desactivadas. La migración
del sitio puede continuar: una diferencia de hashes por sí sola ya no lanza la
excepción que causó el incidente. No se aceptan hashes nuevos sin revisar código.

`before_migrate` y `after_migrate` verifican que no queden hooks IGCTools
incompatibles activos por una mezcla de procesos/cachés. Ese estado inconsistente
sí se rechaza. Los despliegues deben reiniciar web y workers normalmente. Cuando
la fuente coincide, siguen comprobándose campos, PowerPro instalado y dependencias.
La activación del motor nuevo requiere la prueba acotada de la versión instalada.
...
3. En ese entorno, comprobar compatibilidad y activación:
   ...
   Para activar las capas deben coincidir los seis hashes; controlador IGCTools,
   siete rutas IGCTools y paquetes PDF presentes. Si no coincide el origen,
   `status` muestra `activation_blocked: true`, `runtime_enabled: false` y
   `layers_enabled: false`, con los hashes observados y el controlador real.
   Auditar ese código y adaptar la migración; no omitir el control ni modificar
   hashes para pasar. Que `migrate` termine no significa que las capas estén activas.
