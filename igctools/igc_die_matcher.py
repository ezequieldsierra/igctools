...
def _extract_segments(root):
	...
	def walk(node, parent_matrix):
		...
		if tag == "line":
			...
			segments.append({"x1": x1t, "y1": y1t, "x2": x2t, "y2": y2t, "length": length, "type": seg_type})
...
def analyze_die_svg(svg_text):
	try:
		...
		return {"width": None, "height": None, "dx_list": [], "dy_list": []}
	...
	return {"width": width, "height": height, "dx_list": dx_list, "dy_list": dy_list}
...
def find_similar_dies_from_svg(svg_text, tolerance_mm=3.0, max_results=30, tipo_producto=None):
	...
	troquel_filters = {"svg_plano_mecanico_individual": ["is", "set"]}

	if tipo_producto:
		troquel_filters["tipo_producto"] = tipo_producto

	troqueles = frappe.get_all(
		"Troquel", filters=troquel_filters, fields=["name", "svg_plano_mecanico_individual"], limit=500
	)
