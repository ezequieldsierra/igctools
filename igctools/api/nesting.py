...
def _parse_svg_to_paths(svg_str):
	...
	for el in root.iter():
		...
		if t in ("polygon", "polyline"):
			...
			pts = []
			it = iter(coords)
			for xs, ys in zip(it, it, strict=False):
				try:
					x = float(xs)
					y = float(ys)
					pts.append((x, y))
...
def _min_step_y_tetebeche_mm(svg_str, height_mm, gap_y_mm):
	...
	if solid_up.buffer(-CLEAR_TOL_MM).disjoint(solid_down.buffer(-CLEAR_TOL_MM)):
	...
	for _ in range(40):  # precisión sub-micrón en la práctica
		...
		if solid_up.buffer(-CLEAR_TOL_MM).intersects(test.buffer(-CLEAR_TOL_MM)):
			lo = mid
		else:
			hi = mid
