...
def _to_pt(value: str) -> str:
	...
	if unit == "pt":
		return f"{n}pt"
	if unit == "mm":
		return f"{n * MM_TO_PT}pt"
	if unit == "cm":
		return f"{n * CM_TO_PT}pt"
	if unit in ("in", '"'):
		return f"{n * IN_TO_PT}pt"
	if unit == "pc":
		return f"{n * PC_TO_PT}pt"
	if unit == "px":
...
def normalize_svg_root(svg_text: str, width_mm: float, height_mm: float) -> str:
	try:
			...
			if st:
				...
				for chunk in st.split(";"):
					...
					if kl == "stroke-width":
						...
						if not re.match(
							r"^\s*(\d+(?:\.\d+)?(?:\s*,\s*\d+(?:\.\d+)?)*|none)\s*$", v, flags=re.I
						):
							changed = True
						else:
							parts.append(f"{k}:{v}")
		...
		s = re.sub(
			r"<svg", f'<svg width="{float(width_mm)}mm" height="{float(height_mm)}mm"', s, count=1, flags=re.I
		)
		s = re.sub(
			r'stroke-width="\s*([^"]+?)\s*"', lambda m: f'stroke-width="{_to_pt(m.group(1))}"', s, flags=re.I
		)
		s = re.sub(
			r'stroke-width\s*:\s*([^;"]+)', lambda m: f"stroke-width:{_to_pt(m.group(1))}", s, flags=re.I
		)
...
def export_generador_troquel_dxf_v2(
	...
	def _parse_rgb(s: str):
		m = re.match(
			r"^rgba?\(\s*([0-9.]+)\s*,\s*([0-9.]+)\s*,\s*([0-9.]+)(?:\s*,\s*([0-9.]+))?\s*\)\s*$",
			(s or "").strip().lower(),
		)
		if not m:
			return None
		...
		if tag == "line":
			...
			for sp in subs:
				...
				for seg in sp:
					...
					if cls == "arc":
						ok = _dxf_add_arc_from_segment_3pt(
							msp, seg, viewbox, width_mm, height_mm, M, dxfattribs
						)
						if ok:
							continue
