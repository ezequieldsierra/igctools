...
def _validate_bpd_url(url: str) -> str:
    ...
    if parsed.hostname != PRODUCTION_HOST:
        raise ValueError(
            "Host BPD no autorizado para CA personalizada: "
            + str(parsed.hostname)
        )
...
def bpd_ca_bundle():
    ...
    if not _CERTIFICATE_FILE.exists():
        raise RuntimeError(
            "No existe certificado DigiCert requerido: "
...
def _rate_limit_headers(response):
    return {
        ...
        "burst_remaining": response.headers.get(
    ...
    with bpd_ca_bundle() as verify_bundle:
        ...
        if token_http_code < 200 or token_http_code >= 300:
            return {
                ...
                "error": "BPD OAuth respondio HTTP "
                + str(token_http_code),
                "ssl_verification": "ENABLED",
            }
        ...
        if rate_response.status_code == 401:
            ...
            if (
                ...
                or token_response_2.status_code >= 300
            ):
                return {
                    "ok": False,
                    "stage": "TOKEN_REFRESH",
                    "http_code": token_response_2.status_code,
                    "error": "BPD renovacion OAuth respondio HTTP "
                    + str(token_response_2.status_code),
                    "ssl_verification": "ENABLED",
                }
        ...
        if rate_http_code < 200 or rate_http_code >= 300:
            result = {
                ...
                "error": "BPD consultaTasa respondio HTTP "
                + str(rate_http_code),
