...
def _find_master_group(svg_root):
	return _find_first(
		svg_root,
		lambda el: _local_name(el.tag) == "g" and (el.attrib.get("id", "").startswith("gt_master_svg_")),
	)
...
def _find_nest_group(svg_root):
	return _find_first(
		svg_root, lambda el: _local_name(el.tag) == "g" and (el.attrib.get("id", "").startswith("gt_nest_"))
	)
...
def generar_svg_gomas(
	...
	try:
		...
		return {"ok": False, "error": f"No pude parsear el SVG: {type(e).__name__} {e!r}"}
	...
	if vb0 is None:
		...
		return {
			"ok": True,
			"svg": out_svg,
			"debug": {"ms": int((time.time() - t0) * 1000), "note": "no_viewbox"},
		}
	...
	for m in instances:
		...
		try:
			rb = rb.buffer(-open_mm, cap_style=cap_style, join_style=join_style).buffer(
				open_mm, cap_style=cap_style, join_style=join_style
			)
		except Exception:
			pass
		...
		if crease_u is not None and (not crease_u.is_empty):
			try:
				rb = rb.difference(
					crease_u.buffer(max(avc, 0.001), cap_style=cap_style, join_style=join_style)
				)
			except Exception:
				pass
	...
	if rb_all.is_empty:
		...
		return {
			"ok": True,
			"svg": out_svg,
			"debug": {"ms": int((time.time() - t0) * 1000), "note": "rb_union_empty"},
		}
