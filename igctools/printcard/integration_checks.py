...
from igctools.printcard import compatibility, helper, migration

FIXTURES = Path(__file__).resolve().parents[2] / "tests" / "fixtures"


def refresh_runtime_hooks():
...
@contextmanager
def audited_source_fixture():
	"""Install only public source bytes into a temporary import path, never a bench app."""
	assert_disposable_site()
	if importlib.util.find_spec("powerpro") is not None:
		raise RuntimeError("This fixture requires an empty test bench without PowerPro.")
	manifest = json.loads(Path(compatibility.__file__).with_name("origin.json").read_text())
	with TemporaryDirectory(prefix="igc-printcard-public-source-") as folder:
		root = Path(folder)
		with zipfile.ZipFile(FIXTURES / "printcard_powerpro.zip") as archive:
			for name, entry in manifest["files"].items():
				path = root / entry["source"]
				path.parent.mkdir(parents=True, exist_ok=True)
				path.write_bytes(archive.read(name))
		(root / "powerpro/__init__.py").write_text("")
		sys.path.insert(0, folder)
		try:
			refresh_runtime_hooks()
...
class TestPrintCardIntegration(unittest.TestCase):
	...
	def test_unknown_source_does_not_block_pre_schema_updates(self):
		"""Reproduce the failed migration phase with all three reported files changed."""
		assert_disposable_site()
		...
		manifest = json.loads(Path(compatibility.__file__).with_name("origin.json").read_text())
		originals = {}
		custom = frappe.db.get_value("DocType", "PrintCard", "custom")
		saved = frappe.get_all(
			"PrintCard", fields=["name", "archivo", "printcard_file", "printcard_file_signed"]
		)
		try:
			for name in ["controller.py", "helper.py", "permissions.py"]:
				path = self.source_root / manifest["files"][name]["source"]
				originals[path] = path.read_bytes()
				path.write_bytes(originals[path] + b"\n# different installed source\n")
			# A custom test DocType supplies a real Frappe legacy controller without
			# importing or installing another business app in this disposable bench.
			frappe.db.set_value("DocType", "PrintCard", "custom", 1)
			refresh_runtime_hooks()
			self.assertNotEqual(get_controller("PrintCard").__module__, "igctools.printcard.controller")
			self.assertNotIn("PrintCard", frappe.get_hooks("override_doctype_class", app_name="igctools"))
			self.assertFalse(frappe.get_hooks("override_whitelisted_methods", app_name="igctools"))
			self.assertFalse(frappe.get_hooks("permission_query_conditions", app_name="igctools"))
			self.assertEqual(sum(not row["matches"] for row in compatibility.source_status()["files"]), 3)
			# Run the actual phase shown in the production traceback, including its hooks.
			SiteMigration().pre_schema_updates()
			migration.verify_activation()
			report = migration.status()
			self.assertTrue(report["activation_blocked"])
			self.assertFalse(report["layers_enabled"])
			self.assertEqual(
				saved,
				frappe.get_all(
					"PrintCard", fields=["name", "archivo", "printcard_file", "printcard_file_signed"]
			...
			frappe.db.set_value("DocType", "PrintCard", "custom", custom)
			refresh_runtime_hooks()
