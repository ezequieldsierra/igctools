(function () {
	...
	function start_alert_sound() {
		try {
			...
			// Audio is optional; keep the notification usable when playback is unavailable.
		}
	}
	...
	function show_overlay(data) {
		...
		overlay.innerHTML = [
			...
			"<span>Broadcast</span>",
			"</div>",
			'<div class="igc-bcast-title">⚠️ ' +
				frappe.utils.escape_html(data.title || "Mensaje") +
				"</div>",
			timeLabel
				? '<div class="igc-bcast-meta">⏱ ' + frappe.utils.escape_html(timeLabel) + "</div>"
				: "",
			"</div>",
